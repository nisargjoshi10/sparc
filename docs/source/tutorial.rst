...
In this tutorial, we demonstrate how to use **SPARC** to build a ML potential and then later use that to study the dissociation of B-N bond. We will show how to:

1. Prepare input files, also see section :ref:`inputfile`.
2. Prepare data
3. Train DeepMD potential models.
4. Run Machine Learning Molecular Dynamics (ML/MD).
5. Use **active learning** together with **QbC** to adaptively improve the model.
6. Perform analysis and visualize results.

Preparation
-----------

First, create a new working directory and copy the example files:

.. code-block:: bash

   mkdir nh3bh3_example
   cd nh3bh3_example
...
- ``INCAR``: VASP control file for AIMD.
- ``input.yaml``: SPARC configuration.
- ``input.json``: DeepMD training parameters.

YAML Input
----------

The ``input.yaml`` defines the SPARC workflow:

.. literalinclude:: input.yaml
   :language: yaml
   :caption: Example ``input.yaml`` file.

Key sections:

- ``general``: Structure file, MD steps, log frequency.
- ``md_simulation``: Ensemble, thermostat, timestep, temperature.
- ``dft_calculator``: Settings for VASP.
- ``active_learning``: Control for iterative improvement.
