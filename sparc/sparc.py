...
def main():
    ...
    parser.add_argument(
        "-i", "--input_file", type=str, default="input.yaml", help="Input YAML file"
    )
    ...
    system = read(
        config["general"]["structure_file"]
    )  # e.g., POSCAR or xyz file (ASE accepted formats)
    ...
    if dftmd_is or training_is or dpmd_run_is:
        iter_structure = create_iteration_dirs(
            iter_num=0
    ...
    if dftmd_is:
        ...
        SparcLog(
            f" ! ab-initio MD Simulations will be performed at Temp.: {temperature}K !"
        )
        SparcLog(
            "========================================================================"
        )
        ...
        if plumed_is:
            ...
            remove_backup_files(
                file_ext="bck.*"
            )  # remove PLUMED generated backup files if present!
    ...
    if dpmd_run_is:
        ...
        SparcLog("      MULTIPLE MLP-MD SIMULATION STARTING FROM SAME CONFIGURATION!")
        SparcLog(
            "========================================================================"
        )
        ...
        for i in range(n_sample):
            ...
            umbrella_enabled = config["deepmd_setup"]["umbrella_sampling"].get(
                "enabled", False
            )
            ...
            if dp_plumed_is and umbrella_enabled:
                ...
                SparcLog(
                    "       Umbrella Sampling Enabled — Running MLMD Windows with PLUMED       "
                )
                SparcLog(
                    "========================================================================"
                )
        ...
        if config["active_learning"]:
            ...
            if not candidate_found_is:
                ...
                SparcLog(
                    "!                No Candidates Found for Labelling                !"
                )
                SparcLog(
    ...
    if learning_is:
        ...
        if learning_restart:
            iter, i_start, candidates, candidate_found_is, labelled_files = (
        ...
        while candidate_found_is and iter < al_iter:
            ...
            for idx, files in enumerate(labelled_files, start=i_start):
                ...
                if header:
                    ...
                    SparcLog(
                        "!{}!".format(
                            f"[Iteration {iter}] Computing Energy and Forces for Candidates".center(
                                70
                            )
                        )
                    )
            ...
            SparcLog(
                "      MULTIPLE SAMPLE MD-SIMULATION STARTING FROM SAME CONFIGURATION!"
            )
            SparcLog(
                "========================================================================"
            )
            for i in range(n_sample):
                ...
                if dp_plumed_is and not umbrella_enabled:
                    ...
                    plumed_file = config["deepmd_setup"].get(
            ...
            if not candidate_found_is:
                ...
                SparcLog(
                    "                 No More Candidates Found for Labelling.                 "
                )
                SparcLog(
                    "                 End of Active Learning Loop.                             "
                )
