...
def setup_DeepPotential(atoms, model_path, model_name="frozen_model.pb"):
    ...
    try:
        ...
        if potential_energy is not None and forces is not None:
            SparcLog("=" * 72)
            SparcLog(
                f"DeepPotential model successfully loaded and tested: \n {dp_model}"
            )
            SparcLog("=" * 72)
        else:
            SparcLog("=" * 72)
            SparcLog(
                "Error: Failed to compute energy and forces with DeepPotential model"
            )
            SparcLog("=" * 72)
...
def deepmd_training(
    ...
    for i in range(1, num_models + 1):
        ...
        if os.path.exists(dir_name):
            SparcLog(
                f"Training folder '{dir_name}' already exists. Using this folder for training."
            )
        else:
        ...
        try:
            ...
            if os.path.exists("checkpoint"):
                SparcLog("*" * 80)
                SparcLog(
                    "[INFO] Checkpoint file found | Resuming training with the current model state"
                )
                SparcLog("*" * 80 + "\n")

                # Run DeepMD training with the checkpoint
                subprocess.run(
                    ["dp", "train", input_file, "-r", "model.ckpt"], check=True
                )
            else:
            ...
            logger.error(f"An error occurred during training: {e!s}")
        finally:
        ...
        evaluate_model_accuracy(
            f"{dir_name}/{frozen_model_name}", f"{datadir}/validation_data"
        )
