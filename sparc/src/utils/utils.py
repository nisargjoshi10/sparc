...
def log_md_setup(dyn, atoms, dir_name, write_dist=False):
    ...
    with MDLogger(f"{dir_name}/AseMolDyn.log") as log:
        if step == 0:
            log.file.write(
                f"# {'Steps':<6} {'Epot':<10} {'Ekin':<10} {'Total':<10} {'Temp':<6}\n"
            )
        log.file.write(
...
def save_xyz(atoms, trajfile, write_mode, dir_name):
    ...
    trr = TrajectoryWriter(
        filename=traj_file, mode=write_mode, atoms=atoms, properties=properties
    )
    trr.write(atoms)
...
def restart_progress(start_iteration):
    ...
    SparcLog("------------------------------------------------------------------------")
    SparcLog(f" Candidate Folder      | {candidate_dir!s:<35}")
...
def check_physical_limits(atoms, distance_metrics):
    ...
    for check in distance_metrics:
        ...
        if distance < min_distance or distance > max_distance:
            ...
            SparcLog(
                f"  MEASURED: {distance:.2f} Å   (MIN. LIMIT: {min_distance:.2f} Å)  "
            )
            SparcLog(
                f"  MEASURED: {distance:.2f} Å   (MAX. LIMIT: {max_distance:.2f} Å)  "
            )
            SparcLog("=" * 50 + "\n")
